...
function organize_home_directory() {
    ...
    [ -d "Documents" ] && mv -n "Documents" "documents" || { echo_color "RED" "Error: Could not rename Documents."; return 1; }
    echo "Renaming Downloads to downloads..."
    [ -d "Downloads" ] && mv -n "Downloads" "downloads" || { echo_color "RED" "Error: Could not rename Downloads."; return 1; }

    echo "Renaming and moving Desktop to documents/desktop..."
    [ -d "Desktop" ] && mv -n "Desktop" "documents/desktop" || { echo_color "RED" "Error: Could not rename and move Desktop."; return 1; }
    echo "Renaming and moving Music to documents/music..."
    [ -d "Music" ] && mv -n "Music" "documents/music" || { echo_color "RED" "Error: Could not rename and move Music."; return 1; }
    echo "Renaming and moving Pictures to documents/pictures..."
    [ -d "Pictures" ] && mv -n "Pictures" "documents/pictures" || { echo_color "RED" "Error: Could not rename and move Pictures."; return 1; }
    echo "Renaming and moving Videos to documents/videos..."
    [ -d "Videos" ] && mv -n "Videos" "documents/videos" || { echo_color "RED" "Error: Could not rename and move Videos."; return 1; }

    echo "Deleting Public..."
    [ -d "Public" ] && rm -r "Public" || { echo_color "RED" "Error: Could not delete Public."; return 1; }
    echo "Deleting Templates..."
    [ -d "Templates" ] && rm -r "Templates" || { echo_color "RED" "Error: Could not delete Templates."; return 1; }
